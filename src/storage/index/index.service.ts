...
export class IndexService implements IndexApi {
    ...
    create(column: ColumnMeta): void {
        ...
        const values = data.map((document) => ({
            ...
            value:
                column.type === "string"
                    ? document.raw[column.name]
                    : parseInt(document.raw[column.name]),
        }));
    ...
    findOne(column: ColumnMeta, value: string): DocumentId {
        ...
        const valueIndex = this._binarySearch(
            index,
            column.type === "string" ? value : parseInt(value),
        );

        if (valueIndex === null) {
            throw new Error(
                `${IndexService.name}.findOne. Value not found. ${column.name} = ${value}`,
            );
        }
